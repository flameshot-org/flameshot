...
void zbar_image_set_data(zbar_image_t* image,
                         const void* data,
                         unsigned long data_byte_length,
                         void (*cleanup)(zbar_image_t* image));
...
{
    return tr(
      "Scan QR codes & barcodes, open URLs, and generate custom QR codes");
}
...
void QrTool::pressed(CaptureContext& context)
{
    ...
    QImage grayImg =
      capture.toImage().convertToFormat(QImage::Format_Grayscale8);
    ...
    if (scanner) {
        ...
        if (image) {
            ...
            zbar_image_set_data(
              image, grayImg.constBits(), (unsigned long)w * h, nullptr);
    ...
    if (!scannedData.isEmpty()) {
        ...
        AbstractLogger::info() << tr("Decoded %1: %2 (copied to clipboard)")
                                    .arg(symbolTypeName, scannedData);
    } else {
        AbstractLogger::info()

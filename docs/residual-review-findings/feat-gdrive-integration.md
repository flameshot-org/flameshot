...
### R-G. Retention pruning still unlinks cached files outside `History::remove()` — P3 (follow-up improvement)
- **Where:** `src/utils/history.cpp` — `History::history()` removes over-retention thumbnails with its own inline `QFile(...).remove()`, alongside the `History::remove()` added for the delete controls.
- **Finding:** The post-upload-delete fix made `History::remove()` the single removal path *for the two delete controls*, which is what its plan required (R4). Retention pruning was not routed through it, so `history()` remains a second place in `History` that unlinks a cached file. Not a defect — the two have different jobs (pruning walks a listing it already holds; removal resolves one packed name) — but it leaves the "one removal path" property narrower than it reads.
- **Why deferred:** Offered at the end of the delete fix and left out deliberately: it is outside that plan's scope ("both delete controls use it") and its Definition of Done forbids unrelated changes riding along. Future improvement: have `history()` call `remove()` for each over-retention entry, so every unlink in the codebase goes through one function. Cheap and behavior-neutral — `remove()`'s extra `exists()` check is redundant there, nothing more.
